...
class EFC(nn.Module):
    def __init__(self,
        ...
        self.dwconv = nn.Conv2d(c2, c2, kernel_size=3, stride=1, padding=1, groups=c2) # groups=c2: 表示每个输入通道的卷积使用独立的卷积核，即深度卷积。
    ...
    def forward(self, x):
        ...
        X_ = X_.chunk(4, dim=1) # 通过 chunk(4, dim=1) 把特征图在通道维度上分成 4 份。
        out = []
        # 每个分块的交互与重新加权
        for group_id in range(0, 4):
            out_1 = self.interact(X_[group_id])
            N, C, H, W = out_1.size()
            x_1_map = out_1.reshape(N, 1, -1) # (N, C, H, W) 变形为 (N, 1, C*H*W)
            mean_1 = x_1_map.mean(dim=2, keepdim=True) # 对 x_1_map 沿着第 2 个维度（即展平后的维度，表示特征维度）计算均值
            x_1_av = x_1_map / mean_1 # 对 x_1_map 进行归一化处理
            x_2_2 = F.softmax(x_1_av, dim=1)
            x1 = x_2_2.reshape(N, C, H, W)
            x1 = X_[group_id] * x1
            out.append(x1)
        out = torch.cat([out[0], out[1], out[2], out[3]], dim=1)
        N, C, H, W = out.size() # 获取拼接后张量 out 的形状
        x_add_1 = out.reshape(N, self.group_num, -1) # 将 out 张量重塑为形状 (N, self.group_num, -1)
