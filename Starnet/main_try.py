...
class Star(nn.Module):
    def __init__(self, out_planes=4, encoder='starnet_s3'):  # out_planes=1表示输出通道数为1，即二分类问题
        ...
        self.down_conv2 = nn.Sequential(
            nn.Conv2d(32, 128, kernel_size=3, stride=4, padding=1),  # stride=4，输出尺寸是 [1, 128, 14, 14]
            nn.BatchNorm2d(128),
            nn.ReLU(inplace=True)
        )
    ...
    def forward(self, x):
        ...
        e1_gap = F.adaptive_avg_pool2d(x_d1, (1, 1))
        e2_gap = F.adaptive_avg_pool2d(x_d2, (1, 1))
