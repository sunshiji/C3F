...
class Star(nn.Module):
    def __init__(self, out_planes=4, encoder='starnet_s3'):  # out_planes=1表示输出通道数为1，即二分类问题
        ...
        #
        # self.decoder_3 = nn.Sequential(nn.Conv2d(512, 256, 1, bias=False),
        #                            nn.BatchNorm2d(256),
        #                            nn.ReLU(inplace=True),nn.Conv2d(256, out_planes, kernel_size=1, stride=1))
    ...
    def forward(self, x):
        ...
        d1 = F.adaptive_avg_pool2d(x_d3, (1, 1))  # 全局平均池化，输出维度为 (B, C, 1, 1)
