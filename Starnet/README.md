...
python3 -m torch.distributed.launch --nproc_per_node=8 train_imagenet.py --data {path-to-starnet} --model {demonet-variants} -b 256 --lr 4e-3 --model-ema
...
python3 -m torch.distributed.launch --nproc_per_node=8 train_imagenet.py --data {path-to-starnet} --model {starnet-variants} -b 256 --lr 3e-3 --weight-decay 0.05 --aa rand-m1-mstd0.5-inc1 --cutmix 0.2 --color-jitter 0. --drop-path 0.
```
