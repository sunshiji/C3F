...
from main import Star as create_model
...
def main():
    ...
    # load image
    # 指向需要遍历预测的图像文件夹
    imgs_root = r"/home/szh/datasets/SIW-13_1/test/Mongolian"
    # imgs_root1 = ""
    ...
    img_path_list1 = [os.path.basename(i) for i in os.listdir(imgs_root) if i.endswith(".jpg")]

    # read class_indict
    json_path = r'classSIW-13_indices.json'
    ...
    # create model
    # model = Star(out_planes=13,encoder='starnet_s3').to(device)
    # load model weights
    weights_path = r"/home/szh/code/pytorchProject/Rewrite-the-Stars-main/starnet/model_weight/wshHea_SIW-13_1_models/wshHea_SIW-13_1.pth"  # 替换成你训练好的权重路径
    ...
    # model.backbone.head = torch.nn.Linear(in_features=model.backbone.norm.normalized_shape[0], out_features=13)
    # model = model.to(device)
    model = create_model(out_planes=13).to(device)
    model.backbone.head = nn.Linear(in_features=model.backbone.head.in_features, out_features=13)
    model.load_state_dict(torch.load(weights_path, map_location=device), strict=False)
    # prediction
    model.eval()
    ...
    f = open(r'/home/szh/code/pytorchProject/Rewrite-the-Stars-main/starnet/wshHea_SIW-13_1_Mongolian.txt', 'a+')  # 打开一个文件用于写入结果
